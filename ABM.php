	   ...
	   foreach($_POST['nombre_genero'] as $selected){
		  $generos=$generos.' '.$selected;
            ...
            if(mysqli_num_rows($registros)>0){  
			 ...
			 $ar=fopen("destacados.txt","a") or die("No se pudo abrir el archivo");
						fputs($ar,$_POST['titulo'].",");
						fputs($ar,$_POST['puntaje'].",");
						fputs($ar,$generos.";");
                        fclose($ar);
                        $ar=fopen("destacados.txt","r") or die("No se pudo abrir el archivo");
                        while (!feof($ar)){
                           $linea=fgets($ar);
                           echo $linea;
                        }
